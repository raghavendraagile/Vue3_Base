    ...
    >
      ...
      <div class="add_new_button">
        <v-tooltip :text="this.$t('add_new')" location="bottom">
          <template v-slot:activator="{ props }">
            <router-link
              :to="{ name: 'page-builder-amend', query: { s_tab: tabs } }"
              style="color: white"
            >
          ...
          <template v-slot:item="props">
            <tr class="vdatatable_tbody">
              ...
              <td>
                <v-btn
                  size="small"
                  @click="viewPageBuilder(props.item.selectable.slug)"
                  :disabled="loading"
                  class="ma-1"
                  color="blue"
                  >{{ $t("view_en") }}</v-btn
...
export default {
  ...
  computed: {
    ...
    headers_en() {
      let headers = [
        ...
        {
          title: this.$t("parent_page_builder_en"),
          key: "parent_page_builder",
        },
    ...
    headers_ar() {
      let headers = [
        ...
        {
          title: this.$t("parent_page_builder_ar"),
          key: "parent_page_builder",
        },
  ...
  methods: {
    ...
    fetchPageBuilder() {
      this.initval = true;
      this.$axios
        .get(process.env.VUE_APP_API_URL_ADMIN + "fetch-page-builder")
        .then((res) => {
    ...
    statusUpdate() {
      this.loader = true;
      this.$axios
        .post(process.env.VUE_APP_API_URL_ADMIN + "update-page-builder", {
          id: this.status_id,
        })
        .then((res) => {
          if (Array.isArray(res.data.message)) {
